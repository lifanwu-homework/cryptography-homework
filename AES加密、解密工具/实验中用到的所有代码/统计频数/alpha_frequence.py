...
for i in sample_1:
    all+=i
for i in range(MAXN):
    sample_1[i]=1.0*sample_1[i]/all
p1 = plt.bar(range(1, MAXN+1),
             height = sample_1,
             width = 0.3)
...
#构造需要显示的值
X = np.arange(0, MAXN, step=1)#X轴的坐标
Y = np.arange(0, MAXN, step=1)#Y轴的坐标
                           #设置每一个（X，Y）坐标所对应的Z轴的值，在这边Z（X，Y）=X+Y
Z = np.zeros(shape=(MAXN, MAXN))
all_ij=0
for i in range(MAXN):
    for j in range(MAXN):
        Z[i, j] = int(temp[i * MAXN + j])
        all_ij+=Z[i, j]
for i in range(MAXN):
    for j in range(MAXN):
        Z[i, j] =1.0*Z[i, j]/all_ij
